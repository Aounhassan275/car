    ...
    <div class="card">
        ...
        <div class="table-responsive">
            <table class="table" id="datatables-reponsive">
                ...
                <tbody>
                    @foreach (App\Models\Review::all() as $key => $testimonial)
                    <tr> 
                        <td>{{$key+1}}</td>
                        <td>{{$testimonial->name}}</td>
                        <td>{!! $testimonial->description !!}</td>
                        <td class="table-action">
                            <button data-toggle="modal" data-target="#edit_modal" name="{{$testimonial->name}}" 
                                description="{{$testimonial->description}}" id="{{$testimonial->id}}" class="edit-btn btn"><i class="align-middle" data-feather="edit-2"></i></button>
                        </td>
                        <td class="table-action">
...
<script>
    $(document).ready(function(){
        $('.edit-btn').click(function(){
            ...
            $('#id').val(id);
            $('#updateForm').attr('action','{{route('admin.testimonial.update','')}}' +'/'+id);
        });
    });
</script>
<script>
